...
const getColorsList = (
  ...
  const colorsList = [];
  const givenColor = isValidHex(
    numberToHex(hashToObject(colorsObject.scaleValue).mainColor)
  )
    ? numberToHex(hashToObject(colorsObject.scaleValue).mainColor)
    : errorColor;
  ...
  for (step = 0; step < colorsAmount; step++) {
    if (
      isValidHex(numberToHex(hashToObject(colorsObject.scaleValue).mainColor))
    ) {
      colorsList.push(
        Color(givenColor)
...
const defaultState = {
  ...
  r: Color(numberToHex(initialColor))
    .rgb()
    .red(),
  g: Color(numberToHex(initialColor))
    .rgb()
    .green(),
  b: Color(numberToHex(initialColor))
    .rgb()
    .blue()
};
...
class ScaleApp extends Component {
  constructor(props) {
    ...
    this.handleDarkColorsAmountChange = this.handleDarkColorsAmountChange.bind(
      this
    );
    ...
    this.handleLightColorsAmountChange = this.handleLightColorsAmountChange.bind(
      this
    );
    this.handleLightestAmountChange = this.handleLightestAmountChange.bind(
      this
    );
    this.handleLightColorsMixRotate = this.handleLightColorsMixRotate.bind(
      this
    );

    this.handleDarkColorsAmountBlur = this.handleDarkColorsAmountBlur.bind(
      this
    );
    this.handleDarkestAmountBlur = this.handleDarkestAmountBlur.bind(this);
    this.handleLightColorsAmountBlur = this.handleLightColorsAmountBlur.bind(
      this
    );
    this.handleLightestAmountBlur = this.handleLightestAmountBlur.bind(this);
    this.handleLightColorsMixRotateBlur = this.handleLightColorsMixRotateBlur.bind(
      this
    );
    this.handleDarkColorsMixRotateBlur = this.handleDarkColorsMixRotateBlur.bind(
      this
    );
    ...
    this.handleLightSaturationChange = this.handleLightSaturationChange.bind(
      this
    );
    this.handleDarkSaturationChange = this.handleDarkSaturationChange.bind(
      this
    );
  ...
  getHash() {
    ...
    if (hash) {
      ...
      const getHashObject = () => {
        var hashObject = {};
        stateKeysArray.forEach(
          (key, i) => (hashObject[key] = hashValuesArray[i])
        );
  ...
  updateThemeColor() {
    document
      ...
      .setAttribute('content', numberToHex(this.state.mainColor));
  }
  ...
  rgbToMainColor() {
    setTimeout(() => {
      this.setState({
        mainColor: hexToNumber(
          Color(`rgb(${this.state.r}, ${this.state.g}, ${this.state.b})`).hex()
        )
      });
    }, 0);
  }
  ...
  updateRgbWithMainColor(color) {
    if (isValidHex(numberToHex(color))) {
      this.setState({
        r: Color(numberToHex(color))
          .rgb()
          .red(),
        g: Color(numberToHex(color))
          .rgb()
          .green(),
        b: Color(numberToHex(color))
          .rgb()
          .blue()
      });
    }
  ...
  getColorsList(
    ...
    const colorsList = [];
    const givenColor = isValidHex(numberToHex(this.state.mainColor))
      ? numberToHex(this.state.mainColor)
      : errorColor;
    ...
    return (
      <MainWrapper color={numberToHex(this.state.mainColor)}>
        <TopSection>
          <ColorsSection>
            ...
            <ColorBlocksRow style={{ marginBottom: 88 }}>
              {this.getColorsList(
                this.state.darkColorsAmount,
                this.state.darkestAmount,
                'black',
                this.state.darkColorsMixRotate,
                this.state.darkSaturation,
                true
              )
                .reverse()
                .map((color, index) => (
              ...
              <ColorBlock
                wide
                style={{
                  background: isValidHex(numberToHex(this.state.mainColor))
                    ? numberToHex(this.state.mainColor)
                    : errorColor
                }}
              ...
              {this.getColorsList(
                this.state.lightColorsAmount,
                this.state.lightestAmount,
                'white',
                this.state.lightColorsMixRotate,
                this.state.lightSaturation
              ).map((color, index) => (
                <ColorBlock
                  style={{ background: color }}
                  hasValidColor={isValidHex(numberToHex(this.state.mainColor))}
        ...
        <FooterSection>
          ...
          &nbsp; · &nbsp;made by{' '}
          <a href="http://hihayk.com" target="_blank" rel="noopener noreferrer">
            Hayk
          </a>
          &nbsp; · &nbsp;
          <a
            href="https://github.com/hihayk/scale"
            target="_blank"
        ...
        <GalleryItem color={numberToHex(getColorsObject())}>
          <a href={`https://hihayk.github.io/scale/${value.scaleValue}`}>
            <ColorBlocksRow disabled>
              {getColorsList(
                getColorsObject().darkColorsAmount,
                getColorsObject().darkestAmount,
                'black',
                getColorsObject().darkColorsMixRotate,
                getColorsObject().darkSaturation,
                value,
                true
              )
                .reverse()
                .map((color, index) => (
                  <ColorBlock
                    style={{ background: color }}
                    hasValidColor={isValidHex(
                      numberToHex(getColorsObject().mainColor)
                    )}
                    color={color}
                    key={index}
                  />
              ...
              {getColorsList(
                getColorsObject().lightColorsAmount,
                getColorsObject().lightestAmount,
                'white',
                getColorsObject().lightColorsMixRotate,
                getColorsObject().lightSaturation,
                value
              ).map((color, index) => (
                <ColorBlock
                  style={{ background: color }}
